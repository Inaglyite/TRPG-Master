...
```json
{
  "type": "narrative_segments",
  "segments": [
    { "kind": "narration", "text": "雨水顺着办公室的窗户滑落。" },
    { "kind": "speech", "text": "「有些东西……确实会留下来。」", "speaker": { "type": "npc", "id": "butler_gregory", "name": "管家格里高利" } }
  ]
}
```

回合定稿时、`done` 之前发送一次，给出本回合叙述的权威段结构（旁白与发言单元的
有序列表）。客户端应以它覆盖流式期间的实时段布局。该结构同时写入回合记录
`narrative_segments` 字段，随 `turn_branched`/`world_switched` 的 `history` 与
`turn_recovery` 的记录原样携带；旧记录没有此字段时按单段叙述渲染。
