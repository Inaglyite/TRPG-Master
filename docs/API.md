...
桌面模式默认关闭账号门禁。云端设置 `TRPG_REQUIRE_AUTH=1` 后使用 Argon2id 账号、可撤销 HttpOnly Session Cookie、登录限流、Origin/CSRF 校验和世界成员权限；仍须由 Nginx 提供 TLS。WebSocket 握手和每次世界切换都会校验成员权限。
...
未识别的 `type` 会返回 `protocol_error`；非法 JSON text frame 会被忽略。前端使用 Zod 对服务端消息的 `type` 白名单做入口校验，无法识别的消息不会进入业务处理器。
...
前端实际使用的颜色 key 映射见 `frontend/src/theme.ts`。未知 key 或不安全的颜色、字体值会被忽略。
...
| `turn_branch_create` | `turn_id`, `label?` | 从指定决策点的完成回合创建并切换独立世界分支 |
...
1. 读取当前 `ModuleRecord.path/world_state_initial.json` 模板，重建数据库 `world_states.state`，并递增 revision。
...
读档会通过 `WorldStore.restore()` 把数据库 `snapshots.state` 恢复到当前 world，执行 schema 迁移并生成新的 revision；待确认的战斗动作也随完整快照恢复。若读取槽位期间当前 world 已被其他动作更新，服务端发送 revision 过期的 `error`，不会覆盖新状态。随后模型消息加入“基于存档续写、不要重新开场”的指令。
...
公开回合历史包含 `parent_turn_id`；客户端在结果消息上创建分支时使用父回合，从而恢复到本次玩家
行动之前的决策点。
...
自报。入口使用严格 schema，仅允许公开展示字段，额外字段会被剥离。

持久化继续使用兼容字段 `narrative_segments`，分支、世界切换和恢复载荷会同时给出
同内容的 `chat_events`。旧客户端仍可读取 `narrative_segments`；旧记录没有段结构时
按单条守秘人旁白迁移。服务端优先解析显式 NPC id；模型漏掉标签但输出行首
`已知NPC姓名：台词` 时，会根据当前世界的公开人物白名单恢复归因。未知姓名不会创建
人物身份。常见的 `“台词。”法伦说……`、`法伦低声说：“台词。”` 也会在明确出现
已知姓名时恢复；全名会生成不冲突的公开短名（如“布莱斯·法伦”→“法伦”），连续
段落中的“他/她”沿用最近明确人物。短术语引文不会被当作台词，没有人物上下文的档案
引文仍属于守秘人旁白。

旧版 `narrative_segments` WebSocket 消息仍在前端白名单中，仅用于连接旧服务端；新版
服务端不再主动发送。
...
在发送 `done` 前，服务端已经在数据库事务中写完该回合的消息、不可变快照、事件和 completed 状态。
...
- 云端 API 已有账号与世界权限；共享房间运行时、角色占用和事件广播仍未实现。
