...
发（含名称与头像）；`narrative_segments` 在 `done` 前下发权威段结构。回合记录持
久化 `narrative_segments`，分支/恢复/回放原样携带；无该字段的旧消息按单段叙述渲
染。头像来自模组 `asset_map.npcs[id]`（NPC）或角色 JSON 的 `portrait` 字段（调查
员，相对模组 assets 解析），无素材时前端显示姓名首字徽章。

前端：`renderer.ts` 流式期间实时构建段并在定稿时以权威段覆盖；`MessageList` 把段
渲染为叙事块内的发言单元（头像 + 名称行 + 引文），旁白段保留守秘人眉题——不做聊
天气泡。系统/骰点/线索/错误消息不参与发言者体系。
