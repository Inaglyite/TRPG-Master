...
- 玩家输入存在时增加玩家回合计数。
- 根据上轮风险和轮数注入 TIER 提醒。
- `src.action_checks` 对明确搜查、聆听、追踪等动作先做一次权威检定，并立即发送骰子事件。
- `src.discovery` 匹配当前场景中尚未发现的 `discovery_rules`；命中后在第一次模型请求前原子提交线索、`flag_effects`、SAN、NPC 揭示和素材事件。
...
- NPC 首次揭示、场景切换和图片线索加入会产生权威事件；`src.handouts` 按编译后的
  `reveal_on` 规则解析素材，按素材 ID 去重并将图片关联回线索。线索图片一旦展示，对应目录线索会自动归档；模型调用 `show_handout` 只是兼容入口。
...
`clue_discovered` 规则，作者提供的受限 `reveal_on` 作为自由文本兼容兜底。旧存档恢复时只刷新
模组的静态素材元数据，并修复已发现但没有图片关联的线索，不覆盖 HP、场景或其他游戏进度。

线索的 `discovery_rules` 也在该边界内。规则只允许预定义行动意图、目标别名、可选技能门槛、
SAN 严重度和 NPC 揭示，不执行作者代码或任意正则。命中规则的效果先由引擎提交，再把
`resolved_discoveries` 交给模型叙述，因此关键线索与图片不依赖 Function Calling 的自觉性。
...
- [ReAct](https://arxiv.org/abs/2210.03629)适合观察结果会改变后续推理的循环；本地缓存和重复状态读取不属于这种情况。
