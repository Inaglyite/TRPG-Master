...
世界状态 v2 通过 `state_meta.domains` 声明每个聚合的唯一 JSON 路径，不复制可变状态。旧文件状态在导入数据库时经过 `world_migrations`；运行时事实来源是 `world_states.state`。

## 1. 系统定位

TRPG Master 同时支持本地桌面和账号化云端运行。Electron 提供桌面窗口，FastAPI 提供 HTTP/WebSocket 服务。每条连接当前创建一个 `GameEngine`，其中只有一份 `ModelSession`、消息历史和 LangGraph 回合工作流。工作流在普通叙事节点与临时战斗职责节点间路由；这两个节点不是独立 Agent。Python 工具通过 `DatabaseWorldStore` 执行确定性规则与状态写入。

核心边界：

- 模型可以提出工具调用，但不能替代骰子、伤害、SAN、存档等确定性实现。
- `world_states.state` 的 JSONB 文档是一个运行世界的事实来源；生产环境使用 PostgreSQL。
...
- 当前仍不是多人房间服务器：账号、Session 和世界成员权限已经存在，但同一房间尚无共享 GM 历史、行动队列、角色占用与事件广播。
...
flowchart LR
    ...
    Server <--> Engine[One GameEngine + ModelSession]
...
| 回合编排 | `src/agent_graph.py` | 单引擎叙事/战斗职责节点路由、工具循环、finalize |
...
| 世界存储 | `src/database_store.py`、`src/world_migrations.py` | 数据库行锁、revision、事务恢复与 schema 迁移；旧文件 Store 仅供导入/兼容 |
| 回合事件流 | `src/event_stream.py` | 跨线程 FIFO 发送、`turn_id`、单调 `seq` 与回合生命周期 |
| 回合日志 | `src/database_turn_journal.py` | 数据库回合父链、消息/快照、恢复事件、诊断与叙事变体 |
...
工具 CLI 通过 argv 列表和 `shell=False` 启动，并接收 `TRPG_PROJECT_ROOT`、`TRPG_RUNTIME_ROOT`、`TRPG_MODULE` 与 `TRPG_WORLD_ID`。子进程用 `RuntimeContext.from_env()` 打开同一 `DatabaseWorldStore`，不会仅凭模组名猜测唯一状态。
...
- 同一 `world_id` 的数据库状态更新有事务保护，但两个独立连接的 `GameEngine` 仍可能基于不同消息历史续写。
- 账号、世界角色和权限已经存在；尚缺角色占用、同房间事件广播与共享 GM 内存。
...
5. 大型冷快照增长后，把 `snapshots` 内容迁到对象存储并保留数据库索引与哈希。
