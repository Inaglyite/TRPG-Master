...
TRPG Master 同时支持本地桌面和账号化云端运行。Electron 提供桌面窗口，FastAPI 提供 HTTP/WebSocket 服务。每条连接创建一个 `GameEngine`，其中只有一份 `ModelSession`、消息历史和 LangGraph 回合工作流。工作流在普通叙事节点与临时战斗职责节点间路由；这两个节点不是独立 Agent。Python 工具通过 `DatabaseWorldStore` 执行确定性规则与状态写入。
...
- 本服务不是多人房间服务器：账号、Session 和世界成员权限已经存在，但同房间多人能力缺失，并发边界与改造路线见 §11。
...
| 传输适配 | `server.py`、`src/ws_session.py`、`src/ws_router.py`、`src/game_application.py`、`src/asset_payload.py` | HTTP 路由、WebSocket 协议分发、连接生命周期与回合租约、传输无关的游戏用例、素材载荷构建、引擎回调转事件 |
...
异常或取消丢弃缓冲状态；多个连接仍各自持有独立 `GameEngine.messages`，因此这不是共享房间（见 §11）。
...
短轮询，failed/interrupted 才提供自动存档回退。补发粒度是完成回合；多人协议所需的按玩家
事件确认序号属于 §11 的未实现范围。
...
`/ws` 已能通过不同 `world_id` 安全运行隔离世界，但本实现不提供同房间多人游戏：
