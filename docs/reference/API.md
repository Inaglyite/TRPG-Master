...
> 按需参考（2026-09-16 整理）：本文件保留详细说明；现行范围见 [架构](../ARCHITECTURE.md)、[协议](../PROTOCOL.md)、[运维](../OPERATIONS.md) 与 [状态](../STATUS.md)。文中的仓库路径按仓库根目录理解；历史实现/验证记录不代表最新验收。

本文保留公共 HTTP 与 legacy 回合 WebSocket 的详细契约。结构化请求、命令、事件另见
[现行协议入口](../PROTOCOL.md)和 `schemas/structured-play/v1/`；不能将下述 legacy
兼容限制套用到 structured_v1。历史源码核对不替代对拟发布版本的接口测试。
...
`docs/reference/MODULE_FORMAT.md`。
...
diagnostics 中。字段含义与采集边界见 [`ARCHITECTURE.md`](LEGACY_ARCHITECTURE.md) 的“回合性能”。
...
- legacy 回合消息没有统一的 `version`、客户端 request ID 或结构化 error code；GM 回合已有
...
- 不同 `world_id` 的状态和存档互相隔离。不能将多个独立单机连接当作共享多人房间；
  多人必须走房间授权与共享运行时路径。
- legacy 单机回合与结构化命令使用不同恢复机制。结构化已有持久 outbox 与按身份过滤的
  事件恢复；不要沿用早期“共享事件日志不存在”的结论，详情见现行协议入口。
