...
初稿：2026-09-13；保留 M0→M5 演进说明。当前验收状态见 [STATUS](../STATUS.md)，不能从里程碑注记推断已发布。

本文源于历史平台计划，历史计划不再覆盖现行协议。**字段级定义以
`schemas/structured-play/v1/` 的 JSON Schema 为唯一正本**——前端
...
| `tests/test_structured_play_protocol.py` | 双向校验与一致性门禁 |
