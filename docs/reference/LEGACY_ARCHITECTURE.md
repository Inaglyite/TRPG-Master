...
> 按需参考（2026-09-16 整理）：本文件保留旧回合引擎及其演进说明，不是 structured_v1 的运行契约；尤其不要把单引擎和 shadow-only 记忆结论套用到新平台。现行范围见 [架构](../ARCHITECTURE.md)、[协议](../PROTOCOL.md)、[运维](../OPERATIONS.md) 与 [状态](../STATUS.md)。文中的仓库路径按仓库根目录理解；历史实现/验证记录不代表最新验收。
...
状态机在两类升级动作前插入阻塞式玩家决定；决定的载荷字段、选项 ID 与超时语义见 `docs/reference/API.md` 的 `decision_request`：
...
> [`ROADMAP.md`](../archive/2026-08/plans/ROADMAP.md#3-场景化多人-trpg-与模组工坊多人收口后)。
...
共用一个版本号。完整契约见 `docs/reference/MODULE_FORMAT.md`。
...
5. 更新 `docs/reference/API.md`（如果协议变化）及相关 Skill。

### 新增服务端事件

1. 在 `EngineCallbacks` 增加回调或从 `server.py` 直接发送。
2. 在 `frontend/src/ws.ts` 增加事件分发。
3. 把 payload 字段和事件顺序写入 `docs/reference/API.md`。
...
显式指定 `--output` 时。作者侧的诊断与校验规则见 `docs/reference/MODULE_FORMAT.md` 的「校验与诊断」一章。
