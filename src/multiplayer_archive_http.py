...
from .room_runtime import ActionReservationError, GameRoom, RoomManager
...
def register_archive_world_route(
    ...
    *,
    ...
    async def abandon_active_solo_world(world_id: str, request: Request):
        """Let a solo owner give up an idle adventure without settling it.

        Normal DELETE remains deliberately strict for every room type.  This
        explicit action exists only for a private solo world and carries both
        the in-memory room lease and a durable RoomAction lease while the
        archive transaction runs.  Therefore an in-flight generation, a
        pending decision, or a competing process cannot be mistaken for a
        harmless idle game.
        """
        ...
        async with manager.world_lifecycle(world_id):
            ...
            try:
                ...
                try:
                    ...
                    logger.exception("放弃后清理房间运行时失败 world_id=%s", world_id)
                return Response(status_code=204)
            except MultiplayerError as exc:
                ...
                if exc.code == "world_not_found":
                    try:
                        access = check_solo_abandon_access(
                            database_url(), world_id, user.id
                        )
                ...
                if not archive_committed and reservation_attempted:
                    try:
                        ...
                        logger.exception(
                            "放弃失败后清理持久行动租约失败 world_id=%s", world_id
                        )
