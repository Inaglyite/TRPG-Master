...
from src.ai.tools.tool_aux_handlers import register_auxiliary_handlers
from src.ai.tools.tool_policy import (
    ENGINE_INTERNAL_CALLER,
    MODEL_CALLER,
    allows_engine_internal_state_path,
)
from src.ai.tools.tool_runtime import ToolRuntime, UnknownToolError
from src.ai.tools.tool_schema_defs import LOAD_SKILL_TOOL, READ_FILE_TOOL
...
TOOLS = [
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "spec": {
                        "type": "string",
                        "description": "骰子规格，如 d100, 1d6, 2d6, 1d10, 3d10",
                    }
                },
                "required": ["spec"],
            },
    ...
    },
    {
        ...
        "function": {
            "name": "sanity_event",
            "description": "一次性结算恐怖事件：根据场景描述确认严重度、执行SAN检定与损失，并触发关联展示素材。关联素材对应的预设线索及其flag_effects会自动提交；结果会列出auto_committed，禁止再重复提交同一线索或标记。首次目击尸体、超自然现象或其他恐怖事件时只调用本工具一次。",
            "parameters": {
                "type": "object",
                "properties": {
                    "description": {"type": "string", "description": "玩家实际目击的恐怖场景简述"},
                    "severity": {
                        "type": "string",
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {"amount": {"type": "integer", "description": "恢复量"}},
                "required": ["amount"],
            },
        },
    },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "item": {
                        "type": "string",
                        "description": "物品名称或足以唯一匹配物品栏的关键字",
                    },
                    "operation": {
                        "type": "string",
                    ...
                    "amount": {
                        ...
                        "minimum": 1,
                        "maximum": 20,
                        "description": "消耗数量或开枪发数，默认 1",
                    },
                    "reason": {
                        "type": "string",
                        "description": "具体用途，如鸣枪示警、用钥匙开门、喝下药剂",
                    },
                },
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {"item": {"type": "string", "description": "物品名称"}},
                "required": ["item"],
            },
        },
    },
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {"path": {"type": "string", "description": "角色卡 JSON 文件路径"}},
                "required": ["path"],
            },
        },
    },
                ...
                "properties": {
                    ...
                    "initial_action": {
                        ...
                        "properties": {
                            "actor_id": {
                                "type": "string",
                                "description": "当前调查员的稳定 id；旧单机可用 pc",
                            },
                    ...
                    "participants": {
                        ...
                        "items": {
                            ...
                            "properties": {
                                "id": {
                                    "type": "string",
                                    "description": "稳定调查员 id、旧单机 pc 或 NPC id",
                                },
                                "dex": {"type": "integer", "description": "可选 DEX 覆盖值"},
                                "con": {
                                    "type": "integer",
                                    "description": "可选 CON 覆盖值，用于重伤检定",
                                },
                                "fighting_brawl": {
                                    "type": "integer",
                                    "description": "可选斗殴覆盖值",
                                ...
                                "damage_spec": {
                                    "type": "string",
                                    "description": "默认伤害骰，如 1d6",
                                },
                                "ready_firearm": {
                                    "type": "boolean",
                                    "description": "是否已持枪待发；先攻按 DEX+50",
                                },
                            },
                            "required": ["id"],
                        },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "actor_id": {
                        "type": "string",
                        "description": "必须等于 combat_status.current_actor",
                    },
                    "target_id": {
                        "type": "string",
                        "description": "攻击目标 id；移动或其他动作可省略",
                    },
                    "action_type": {
                        "type": "string",
                        "enum": ["melee", "firearm", "threat", "move", "other"],
                    },
                    "description": {
                        "type": "string",
                        "description": "具体动作意图，供确认弹窗和后续叙事使用",
                    },
                    "skill": {
                        "type": "string",
                        "description": "可选技能 id，近战默认 fighting_brawl，射击默认 firearms_handgun",
                    },
                    "weapon": {
                        "type": "string",
                        "description": "射击所用武器名称或物品栏中的关键字；用于从“武器（N发）”中扣减弹药",
                    },
                    "damage_spec": {
                        "type": "string",
                        "description": "命中后的伤害骰，如 1d3、1d6、1d8+1",
                    },
                    "damage_mode": {
                        "type": "string",
                        "enum": ["normal", "impaling", "blunt"],
                        "description": "极难成功伤害模式；枪弹/刺击用 impaling，钝击用 blunt",
                    },
                    "defender_choice": {
                        "type": "string",
                        "enum": ["dodge", "fight_back", "take_cover", "no_defense"],
                        "description": "仅用于 NPC 防御选择；PC 防御必须留空，由工具向玩家确认",
                    },
                    "bonus_dice": {"type": "integer", "minimum": 0, "maximum": 2},
                    "penalty_dice": {"type": "integer", "minimum": 0, "maximum": 2},
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "dc": {
                        "type": "integer",
                        "description": "难度等级 DC（5=琐碎 10=简单 15=中等 20=困难 25=极难）",
                    },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "scene_id": {
                        "type": "string",
                        "description": "场景ID，如 entrance_hall, east_wing_parlor",
                    },
                    "description": {
                        "type": "string",
                        "description": "完整的场景描写文本（直接写自然语言，不需要 JSON 转义）",
                    },
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "title": {
                        "type": "string",
                        "description": "结局标题，如「真相大白」「疯狂之末」",
                    },
                    "summary": {
                        "type": "string",
                        "description": "结局概要，200字以内，总结整个冒险的关键发现和最终命运",
                    },
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "description": {
                        "type": "string",
                        "description": "场景简述，如'玩家第一次看到血肉模糊的尸体''NPC突然表现出超自然特征'",
                    }
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "entity_id": {
                        "type": "string",
                        "description": "实体ID（NPC ID/场景ID/线索ID）",
                    },
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "reasoning": {
                        "type": "string",
                        "description": "关联推理描述，如'血迹方向指向楼梯口，管家恰好对楼梯口存在记忆空白'",
                    },
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "name": {
                        "type": "string",
                        "description": "特质名称。恐惧症格式如'墨水恐惧——对任何黑色液体的非理性恐惧'。必须与游戏中的触发事件相关。",
                    },
                    "context": {
                        "type": "string",
                        "description": "触发/来源背景，如'在地下室目睹怪物从墨水中显形后产生'",
                    },
                },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "npc_id": {
                        "type": "string",
                        "description": "NPC ID，如 butler_gregory, lady_elizabeth",
                    },
                    "tier": {"type": "integer", "description": "揭示层级：1/2/3"},
                    "entry_text": {
                        "type": "string",
                        "description": "揭示的具体信息，如'管家在提及楼梯口时表现出异常紧张'",
                    },
                },
                "required": ["npc_id", "tier", "entry_text"],
            },
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                "type": "object",
                "properties": {"npc_id": {"type": "string", "description": "NPC ID"}},
                "required": ["npc_id"],
            },
        },
    },
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "section": {
                        "type": "string",
                        "description": "要更新的字段：goals_and_plans / hidden_facts / inference_notes",
                    },
...
def _sanity_restore(args: dict, context: RuntimeContext) -> str:
    ...
    def restore(_sanity, world: dict) -> dict:
        ...
        return {"restore": amount, "san_before": before, "san_after": pc["san"], "max_san": maximum}
...
def _sanity_check(_args: dict, context: RuntimeContext) -> str:
    ...
    maximum = 99 - int(pc.get("skills", {}).get("cthulhu_mythos", 0))
    return _json_result(
        {"san": san, "max_san": maximum, "ratio": round(san / maximum, 2) if maximum else 0}
    )
...
def _combat_end(args: dict, context: RuntimeContext) -> str:
    return _combat_mutation(context, lambda world: end_combat(world, str(args.get("reason", ""))))
...
def _get_npc_secret(args: dict, context: RuntimeContext) -> str:
    # Keep the registration only so old saved histories receive a deterministic
    # response.  Full NPC objects must not be materialized as ordinary tool
    # results; future private behavior projections get their own typed path.
    del args, context
    return "[错误] get_npc_secret 已停用；完整 NPC 私密对象不可作为工具结果读取"
...
def _state_add_clue(args: dict, context: RuntimeContext) -> str:
    ...
    return _state_command(
        ...
        owner_investigator_id=(pc.get("investigator_id") if visibility == "private" else None),
    )
...
def _create_character(args: dict, context: RuntimeContext) -> str:
    ...
    safe_name = (
        "".join(
            char for char in str(character.get("name", "调查员")) if char.isalnum() or char in "_ "
        ).strip()
        or "investigator"
    )
...
def execute_function(
    ...
    *,
    ...
    caller: str = ENGINE_INTERNAL_CALLER,
) -> str:
    """Execute a trusted engine capability with a narrow caller/path policy.

    Model-originated calls are authorized in ``agent_graph`` against a frozen
    request snapshot.  This guard prevents an accidental future direct call
    from reopening the generic state/file capabilities as a second path.
    """
    # All model-originated calls must enter through agent_graph, where their
    # request-scoped snapshot and schema are verified.  ``execute_function``
    # is deliberately not a second model execution path.
    if caller == MODEL_CALLER:
        return _json_result({"ok": False, "error": "model_tool_forbidden"})
    if caller != ENGINE_INTERNAL_CALLER:
        return _json_result({"ok": False, "error": "unknown_tool_caller"})
    if name in {"state_get", "state_set"} and not allows_engine_internal_state_path(name, args):
        return _json_result({"ok": False, "error": "engine_state_path_forbidden"})
...
def dice_summary(output: str) -> str | None:
    ...
    if "d100_roll" in data:
        ...
        return f"🎲 【{skill}】d100={roll}{dice_note} vs {value} → {level_emoji.get(level, level)}"
