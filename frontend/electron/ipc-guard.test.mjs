...
describe("isTrustedSenderUrl（打包模式）", () => {
  ...
  it("拒绝其他 file:// 与所有远程页面", () => {
    ...
    expect(isTrustedSenderUrl("file:///app/dist/other.html", packaged)).toBe(
      false,
    );
    expect(
      isTrustedSenderUrl("file:///app/dist/index.html.evil", packaged),
    ).toBe(false);
    expect(isTrustedSenderUrl("https://trpg.example.com/", packaged)).toBe(
      false,
    );
    expect(isTrustedSenderUrl("data:text/html,<html></html>", packaged)).toBe(
      false,
    );
  });
...
describe("isTrustedSenderUrl（开发模式）", () => {
  it("只允许 dev server origin", () => {
    ...
    expect(isTrustedSenderUrl("http://127.0.0.1:5173/src/main.tsx", dev)).toBe(
      true,
    );
...
describe("isNavigationAllowed", () => {
  ...
  it("未批准时拒绝云端 origin，批准后放行其下页面", () => {
    expect(
      isNavigationAllowed("https://trpg.example.com/?mode=online", packaged),
    ).toBe(false);
    const approved = {
      ...packaged,
      approvedCloudOrigin: "https://trpg.example.com",
    };
    expect(
      isNavigationAllowed("https://trpg.example.com/?mode=online", approved),
    ).toBe(true);
    expect(
      isNavigationAllowed("https://trpg.example.com/ws/room", approved),
    ).toBe(true);
    expect(isNavigationAllowed("https://evil.example.com/", approved)).toBe(
      false,
    );
  });
  ...
  it("拒绝其他 file:// 与非法 URL", () => {
    ...
    expect(isNavigationAllowed("file:///app/dist/other.html", packaged)).toBe(
      false,
    );
  ...
  it("开发模式显式批准云端后仍只允许该云端 origin 的导航与重定向", () => {
    const approved = {
      ...dev,
      approvedCloudOrigin: "https://trpg.example.com",
    };
    expect(
      isNavigationAllowed("https://trpg.example.com/login", approved),
    ).toBe(true);
    expect(isNavigationAllowed("https://evil.example/redirect", approved)).toBe(
      false,
    );
  });
});
...
describe("isApprovedCloudSenderUrl", () => {
  it("只接受当前批准云端 origin 下的页面", () => {
    ...
    expect(
      isApprovedCloudSenderUrl("https://evil.example/", "https://game.example"),
    ).toBe(false);
