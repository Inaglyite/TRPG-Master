...
if [[ -e "$release" ]]; then
    ...
    runuser -u trpgdeploy -- "$release/.venv/bin/pip" install --disable-pip-version-check -r "$release/requirements.txt"
fi

ln -sfn "$release" "$root/current.next"
mv -Tf "$root/current.next" "$root/current"

if ! systemctl restart trpg-master.service; then
    if [[ -n "$previous" && -d "$previous" ]]; then
        ln -sfn "$previous" "$root/current.next"
        mv -Tf "$root/current.next" "$root/current"
        systemctl restart trpg-master.service || true
    fi
    exit 1
fi
...
journalctl -u trpg-master.service --no-pager -n 80 >&2
if [[ -n "$previous" && -d "$previous" ]]; then
    ln -sfn "$previous" "$root/current.next"
    mv -Tf "$root/current.next" "$root/current"
    systemctl restart trpg-master.service || true
